...
import { roleGuard } from './role.guard'; // Ajusta la ruta
...
describe('roleGuard', () => {
  ...
  it('debería retornar true si el rol es ROLE_ADMIN', () => {
  ...
  it('debería retornar true si el rol es ADMIN', () => {
    authServiceSpy.getRealRole.mockReturnValue('ADMIN');
  ...
  it('debería redirigir a /candidate/dashboard y retornar false si tiene otro rol', () => {
    ...
    expect(result).toBe(false);
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/candidate/dashboard']);
  });
});